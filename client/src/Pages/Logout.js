...
function Logout() {
    ...
    return ( <
        ...
        <
        div className = "flex w-full h-full max-w-7xl bg-white shadow-lg" > { /* Left side: Image */ } <
        div className = "w-1/2 h-full relative" > { /* Image Background */ } <
        img src = { loginBg } // Replace with your image URL
        alt = "Sample"
        ...
        <
        h2 className = "text-center text-2xl font-semibold text-gray-100 mb-6" > { isLoggedIn ? "Enter OTP to Logout" : "Enter OTP to Login" } <
        /h2>
        ...
        button className = "mt-6 w-full py-2 text-3xl font-semibold text-white rounded-lg bg-[#36BCBA] hover:bg-pink-500 transition duration-300"
...

        {
            logoutTime && ( <
                ...
                p > Logout Time: { logoutTime } < /p> {
                    sessionDuration && ( <
                        p > Session Duration: { sessionDuration }
                        hours < /p>
                    )
                } <
                /div>
